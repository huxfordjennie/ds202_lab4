...
```{r}
is.factor(offense$Name)
is.factor(offense$Opponent_Opponent)
is.factor(defense$Name)
is.factor(defense$Opponent_Opponent)
is.factor(bio$Name)
```
...
```
The rare defensive skills are turnovers from fumble returns, forced fumbles and interceptions, along with sacks. 
...
```{r}
WVdef <- defClean %>% filter(Opponent_Opponent == "West Virginia", Statistic == "Tackles_Solo")
KSdef <- defClean %>% filter(Opponent_Opponent == "Kansas State", Statistic == "Tackles_Solo")
```

```{r}
new<- defClean %>% filter(Opponent_Opponent %in% c("Kansas State", "West Virginia"), Statistic == "Tackles_Solo")
new<- new %>% extract(Opponent_Opponent, c("Kansas State", "West Virginia"), c("Kansas State", "West Virginia"))
```


```{r}
ggplot(new aes(x = WVdef, y = KSdef)) +
  geom_point
```
